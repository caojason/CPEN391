...
void inputATCommand(const char *);
...
int getcharBT( void )
{
	...
	unsigned char data = BT_ReceiverFifo;
...
void inputATCommand(const char *command){
	int index = 0;
	while(command[index] != 0){
		putcharBT(command[index]);
		index++;
	}
	putcharBT('\r');
	putcharBT('\n');
...
int main(void){
	...
	int numOfCommands = 1;
	const char *commands[numOfCommands];
	commands[0] = "AT+UART=9600,0,0";

	for(int i = 0; i < numOfCommands; i++){
		inputATCommand(commands[i]);
		printf("%s\n", commands[i]);
		printResponse();
	}
