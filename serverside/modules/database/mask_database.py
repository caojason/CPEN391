...
def insert_table_population(location, count):
    ...
    cursor = db.cursor()
    sql = "INSERT INTO mask_data (location, count, year, month, day, hour, minute, weekday) VALUES (%s, %d, %d, %d, %d, %d, %d, %d)"
    ...
    for i in range(weekday):
        ...
        if day_in_week < 0: 
            if month > 1: 
                ...
                sql = "SELECT * FROM mask_data WHERE location = '{}' AND day = {} AND month = {} year = {}".format(location, day_in_week, month - 1, year)
                cursor.execute(sql)
                result = cursor.fetchall()
                for row in result:
                    weekday = row[7]
                ...
                sql = "SELECT * FROM mask_data WHERE location = '{}' AND day = {} AND month = {} year = {}".format(location, day_in_week, 12, year)
                cursor.execute(sql)
                result = cursor.fetchall()
                for row in result:
                    weekday = row[7]
    ...
    for i in range(1, 8 - weekday): 
        ...
        if day_in_week > calendar.monthrange(year, month)[1]: 
            if month < 12:
                ...
                day_in_week -= calendar.monthrange(year, month)[1]
                sql = "SELECT * FROM mask_data WHERE location = '{}' AND day = {} AND month = {} year = {}".format(location, day_in_week, 1, year + 1)
                cursor.execute(sql)
                result = cursor.fetchall()
                for row in result:
                    weekday = row[7]
