...
void Init_BT(void)
{
	...
	BT_DivisorLatchMSB = divisorVal >> 8;
...
int main(void){
	int x = 0, y = 0, z = 0, xCarry = 0, zCarry = 0;
...
//	unsigned char input[HEIGHT][WIDTH][DEPTH];
	unsigned char output[HEIGHT][WIDTH][DEPTH];
	...
	while ( !(x == WIDTH && y == HEIGHT && z ==DEPTH) ){
		output[y][x][z] = getcharBT() * 2;
		printf("Received %d at %d, %d, %d\n", output[y][x][z], x, y, z);
		z++;
		zCarry = z / DEPTH;
		z %= DEPTH;
	...
	printf("start sending image back\n");

	// send data back
	for(y = 0; y < HEIGHT; y++){
		for(x = 0; x < WIDTH; x++){
			for(z = 0; z < DEPTH; z++){
				putcharBT((int)(output[y][x][z] / 2));
			}
		}
	}
	printf("Finished sending image back\n");

	int timeUsed = time(NULL) - start;

	printf("Took %d seconds\n", timeUsed);

	// start test code for acceleration-------------------------------------------------------------------
	...
	// int output = (int)round(*(gaussian_accel_base) / component_factor);
	// int test = (int)round(10.5);

	// printf("Output is %d\n", output);
	// printf("test output: %d\n", test);
	// end test code for acceleration---------------------------------------------------------------------
